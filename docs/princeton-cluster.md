...
## Remote display: X11 forwarding (default), tuned
...
- **TurboVNC** (ships as a module on the vis nodes): renders server-side,
  streams compressed pixels, normal desktop focus behaviour.
  ```bash
  ...
  # laptop: tunnel 5900 + display number, point any VNC client at it
  ssh -N -L 5905:localhost:5905 <netid>@stellar-vis1.princeton.edu
  # inside the VNC desktop's terminal:  module load tokeye && tokeye
  # done:  vncserver -kill :5
  ```
- **The web app** (`tokeye app` + port tunnel) — rendering happens in your
  local browser, so nothing heavy crosses the wire.
