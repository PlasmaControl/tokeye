...
def get_files(
    ...
    shot_numbers = [
        int(f.strip()) for f in shots_path.read_text().splitlines()
        ]
    valid_shots = [
        num for num in shot_numbers 
        if (fusion_data_dir / f'{num}.h5').exists()
        ]
    valid_shots.sort()
    return valid_shots
...
def load_data(
    ...
    for shot in tqdm(valid_shots, desc='Processing Stage 0'):
        process.pipeline(
            shot, cfg=faith_cfg, out_dir=output_dir, 
            override=settings['overwrite']
            )
