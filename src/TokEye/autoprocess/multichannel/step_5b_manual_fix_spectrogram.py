...
class SpectrogramEditor:
    ...
    def __init__(self, settings: dict):
        ...
        # Stroke preview
        self.stroke_preview_mask = None  # Temporary mask showing current stroke in progress
    ...
    def build_canvas_panel(self, parent):
        ...
        self.canvas.bind('<Motion>', self.on_mouse_move)  # Track mouse for cursor circle
        self.canvas.bind('<Leave>', self.on_mouse_leave)  # Clear cursor when leaving canvas
    ...
    def build_minimap_panel(self, parent):
        ...
        # Edited status label
        self.edited_status_label = ttk.Label(minimap_frame, text="", foreground="orange")
    ...
    def load_data(self, source='auto'):
        ...
        try:
            ...
            if source == 'input':
                ...
                mask_path = self.settings['output_dir'] / f'{shotn}_{channel_idx}_mask.joblib'
            else:  # source == 'auto'
                # Prefer output directory if it exists
                edited_mask_path = self.settings['output_dir'] / f'{shotn}_{channel_idx}_mask.joblib'
                if edited_mask_path.exists():
                    mask_path = edited_mask_path
            
            # Load data
            self.spectrogram = joblib.load(img_path)  # Shape: (F, W) - already magnitude
    ...
    def _navigate(self, direction: int):
        ...
        current_channel_idx = channels.index(self.current_channel) if self.current_channel in channels else 0
        ...
        if 0 <= new_channel_idx < len(channels):
            ...
            if 0 <= new_shot_idx < len(self.shot_numbers):
                ...
                new_shot_channels = self.get_available_channels(self.shot_numbers[new_shot_idx])
    ...
    def _apply_mask_overlay(self, base_img: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        return np.where(mask_binary[:, :, np.newaxis],
    ...
    def render_stroke_preview(self):
        ...
        preview_alpha = 0.6
        img_rgb = np.where(preview_binary[:, :, np.newaxis],
                          ((1 - preview_alpha) * img_rgb + preview_alpha * preview_color).astype(np.uint8),
                          img_rgb)
        ...
        self.canvas.create_image(x1_px, y1_px, anchor=tk.NW, image=viewport_photo, tags='viewport_preview')
    ...
    def update_edited_status(self):
        ...
        output_path = self.settings['output_dir'] / f'{self.current_shot}_{self.current_channel}_mask.joblib'
        
        if output_path.exists():
            self.edited_status_label.config(text="✓ Previously edited", foreground="green")
        else:
    ...
    def render_minimap(self):
        ...
        thumbnail = self.display_image.resize((thumb_width, thumb_height), Image.Resampling.LANCZOS)
    ...
    def on_mouse_drag(self, event):
        ...
        if not self.drawing or self.mask is None or self.last_x is None or self.last_y is None:
            return
    ...
    def on_mouse_move(self, event):
        ...
        self.cursor_position = (self.canvas.canvasx(event.x), self.canvas.canvasy(event.y))
    ...
    def restore_original(self):
        ...
        if not output_path.exists():
            messagebox.showinfo("Info", "No edited version exists - already showing original")
            return
