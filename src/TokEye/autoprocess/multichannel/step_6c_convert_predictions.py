...
def apply_refinement_logic(prediction, input_mask, mean_threshold=0.25, entropy_threshold=0.45):
    """
    ...
    refined_coherent_modes = np.logical_and(
        binary_coherent_mean, 
        binary_coherent_entropy
    )
    refined_coherent_modes = np.logical_and(
        refined_coherent_modes, 
        np.logical_not(np.logical_or(
            input_mask_binary[1], 
            binary_transient_mean
        ))
    )
    refined_coherent_modes = np.logical_or(
        refined_coherent_modes, 
        input_mask_binary[0]
    )
