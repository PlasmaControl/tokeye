...
from lightning.pytorch.callbacks import EarlyStopping, ModelCheckpoint, BasePredictionWriter
...
class DataModule(L.LightningDataModule):
    ...
    def train_dataloader(self):
        return self._create_dataloader(self.train_dataset, shuffle=True, persistent_workers=True)
    
    def val_dataloader(self):
        return self._create_dataloader(self.val_dataset, shuffle=False, persistent_workers=False)

    def predict_dataloader(self):
        return self._create_dataloader(self.pred_dataset, shuffle=False, persistent_workers=False)
    ...
    def predict_step(self, batch, batch_idx):
        with torch.inference_mode():
            ...
            if mc_samples > 1 and save_uncertainty:
                ...
                entropy = -(y_hat_mean_clipped * torch.log(y_hat_mean_clipped) + 
...
class HDF5PredictionWriter(BasePredictionWriter):
    ...
    """
    def __init__(self, hdf5_path, fold_idx, n_samples, save_uncertainty=False, write_interval='batch'):
    ...
    def on_predict_start(self, trainer, pl_module):
        ...
        try:
            ...
            logger.info(f"Fold {self.fold_idx}: Opened HDF5 file for batch prediction writing")
    ...
    def _initialize_datasets(self, sample_shape):
        ...
        logger.info(f"Fold {self.fold_idx}: Initialized preallocated datasets with shape {(self.n_samples,) + sample_shape}")
    ...
    def write_on_batch_end(
        ...
        if self.fold_group is None:
            raise RuntimeError("HDF5 fold group not initialized. on_predict_start may have failed.")
        ...
        if self.batch_idx % 10 == 0:
            logger.info(f"Fold {self.fold_idx}: Saved {self.total_samples}/{self.n_samples} predictions...")
    ...
    def on_predict_end(self, trainer, pl_module):
        ...
        try:
            if self.hdf5_file is not None:
                self.hdf5_file.close()
                logger.info(f"Fold {self.fold_idx}: Successfully saved {self.total_samples} total predictions and closed file")
        except Exception as e:
...
def main(config_path=None):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        logger.info(f"Running predictions for fold {fold_idx} (batch writing to preallocated HDF5 arrays)")
