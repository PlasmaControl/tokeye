...
def get_image_files(step_dir):
    ...
    return sorted([f for f in Path(step_dir).glob('*.tif') if not f.stem.endswith('_masks')], 
...
def convert_nnunet(step_9_dir=None, output_base_dir=None, dataset_name="Dataset001_Binary", overwrite=True, max_workers=None):
    """Convert step 9 outputs to nnUNet format."""
    step_9_dir = Path(step_9_dir or STEP_9_DIR)
    dataset_dir = setup_dir(Path(output_base_dir or CACHE_DIR / 'nnunet') / dataset_name, overwrite)
...
def convert_omnipose(step_9_dir=None, step_10_dir=None, output_base_dir=None, dataset_name="default_dataset", overwrite=True, max_workers=None):
    """Convert step 9 images + step 10 instance masks to Omnipose format."""
    step_9_dir = Path(step_9_dir or STEP_9_DIR)
    step_10_dir = Path(step_10_dir or STEP_10_DIR)
    dataset_dir = setup_dir(Path(output_base_dir or CACHE_DIR / 'omnipose') / dataset_name, overwrite)
...
def main(config_path=None):
    ...
    if config_path is None: settings = default_settings
    else: settings = load_settings(config_path)
