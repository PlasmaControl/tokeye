...
class Module(L.LightningModule):
    ...
    def _shared_step(self, batch, stage):
        ...
        """
        x, y = batch
        y_hat = self(x)  # forward returns the U-Net output tensor directly
...
def make_fold_indices(n_samples: int, n_folds: int, seed: int = 42):
    """K-fold (train_idx, val_idx) splits; val sets partition range(n_samples)."""
...
def _train_one_fold(settings, train_indices, val_indices, out_dir, data_files):
    """Train a single surrogate on the given split; save best_model.ckpt + TorchScript."""
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=out_dir, filename="best_model", monitor="val_loss", mode="min", save_top_k=1
    )
    early_stopping_callback = EarlyStopping(
        monitor="val_loss", patience=settings["early_stopping_patience"], mode="min"
    )
    trainer = L.Trainer(
    ...
    logger.info(f"Training fold -> {out_dir} ({len(train_indices)} train / {len(val_indices)} val)")
    trainer.fit(model, datamodule)

    best_model_path = out_dir / "best_model.ckpt"
    ...
    # Export to TorchScript for inference. Trace a FRESH uncompiled U-Net loaded
    # with the trained weights -- torch.jit.trace cannot trace a torch.compile'd
    # (dynamo-optimized) module, so we never trace model.unet directly.
    ...
    trained_sd = model.unet.state_dict()
    trained_sd = {k.replace("_orig_mod.", ""): v for k, v in trained_sd.items()}
    raw_unet.load_state_dict(trained_sd)
    raw_unet.to(device).eval()

    sample_shape = tif.imread(data_files[0]).shape
    if len(sample_shape) == 2:
        sample_shape = (1,) + sample_shape
    example_input = torch.randn(1, *sample_shape).to(device)
    with torch.no_grad():
        script = torch.jit.trace(raw_unet, example_input)
    torch.jit.save(script, str(out_dir / "final.torchscript.pt"))
    logger.info(f"fold done: {best_model_path}")


def main(config_path=None, settings=None):
    # Merge provided settings over defaults so orchestrator need only pass overrides.
    loaded = {} if settings is not None else (
        {} if config_path is None else load_settings(config_path)
    )
    settings = {**default_settings, **loaded, **(settings or {})}
    settings["input_dir"] = Path(settings["input_dir"])
    settings["model_dir"] = Path(settings["model_dir"])
    ...
    logger.info(f"Found {n_samples} data files; n_folds={n_folds}")

    if n_samples == 0:
        raise RuntimeError(f"No *_img.tif found in {input_dir}")

    if n_folds <= 1:
        all_indices = list(range(n_samples))
        _train_one_fold(settings, all_indices, all_indices, model_dir, data_files)
        return

    folds = make_fold_indices(n_samples, n_folds, seed=settings.get("seed", 42))
    for k, (train_idx, val_idx) in enumerate(folds):
        logger.info(f"\n{'=' * 60}\nFold {k + 1}/{n_folds}\n{'=' * 60}")
        _train_one_fold(settings, train_idx, val_idx, model_dir / f"fold_{k}", data_files)
